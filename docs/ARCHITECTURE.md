# Positive Change Institute LLC

> **Where visionary strategy meets autonomous intelligence — shaping the future of digital ecosystems, one self-evolving innovation at a time.**

[![CI](https://github.com/critter2881/Positive-Change-Institute-LLC/actions/workflows/ci.yml/badge.svg)](https://github.com/critter2881/Positive-Change-Institute-LLC/actions/workflows/ci.yml)
![Python 3.11+](https://img.shields.io/badge/python-3.11%2B-blue)
![License MIT](https://img.shields.io/badge/license-MIT-green)

---

## Overview

Positive Change Institute LLC develops fully automated, AAA-grade turnkey digital solutions including XRPL and Solana token economies, NFT ecosystems with auto-evolving dynamics, and cross-platform dApps — all designed to transform digital ecosystems into living, scalable businesses.

Founded and led by **Christopher S. Rowland Sr.**, the Institute leverages proprietary AI pipelines combining GPT, Grok, and multi-intelligence architectures to orchestrate sophisticated, self-optimizing digital ecosystems.

---

## Quick Start

### Prerequisites

- Python 3.11 or later
- pip

### Installation

```bash
# Clone the repository
git clone https://github.com/critter2881/Positive-Change-Institute-LLC.git
cd Positive-Change-Institute-LLC

# Create and activate a virtual environment
python -m venv .venv
source .venv/bin/activate   # Windows: .venv\Scripts\activate

# Install dependencies
pip install -r requirements.txt

# Copy and configure environment variables
cp .env.example .env
```

### Run the backend

```bash
python backend/app.py
# API available at http://127.0.0.1:5000
```

### Run the tests

```bash
python -m pytest tests/ -v
```

---

## Repository Structure

```
Positive-Change-Institute-LLC/
├── backend/                  # Flask REST API (app factory)
│   └── app.py
├── frontend/                 # Static landing page
│   └── index.html
├── config/                   # Single-source-of-truth registries
│   ├── wallet_registry.json  # Canonical wallet definitions
│   └── divisions_registry.json # Canonical division + product-ID map
├── arcana_enterprise_nfts/   # NFT collections and tier templates
│   ├── arcana_nfts.py
│   └── config/
│       └── tier_templates.yaml
├── tests/                    # pytest suite
│   ├── test_arcana_nfts.py
│   └── test_backend.py
├── docs/                     # Project documentation
│   ├── API.md
│   ├── ARCHITECTURE.md
│   ├── CONTRIBUTING.md
│   ├── PCI_BRAND.md
│   ├── README.md
│   └── SETUP.md
├── .github/workflows/        # GitHub Actions CI/CD
├── .env.example              # Environment variable reference
├── requirements.txt          # Python dependencies
├── LICENSE
└── CHANGELOG.md
```

---

## API Endpoints

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/health` | Liveness / readiness probe |
| `GET` | `/api/divisions` | All registered divisions and product IDs |
| `GET` | `/api/wallets` | Public wallet registry |
| `GET` | `/api/product_metadata` | Metadata for a wallet + product ID pair |
| `GET` | `/api/real_time_liquidity` | Simulated liquidity pool depths |

See [`API.md`](API.md) for full request/response details.

---

## Enterprise Divisions

The Institute operates 14 sovereign liquidity verticals:

| Division | Focus |
|----------|-------|
| Quantum AI : Market Liquidity Engine | AI-driven market intelligence |
| Arcane Blockchain : Token Liquidity Layer | Cross-chain token infrastructure |
| HyperSaaS : Gamified Liquidity Platforms | Engagement-first SaaS |
| XRPL : NFT Liquidity Ecosystem | XRP Ledger NFTs |
| MID25 : Tokenomics Suite | Token design and modeling |
| MNR26 : Reset Dashboard | Protocol rebalancing mechanics |
| ELF : Meme Coin : Viral Liquidity | Meme-economy assets |
| CryptoArcana : QSYS Liquidity Nodes | Quantum-inspired node infrastructure |
| Arcanex : Stake & Liquidity Optimizer | Automated staking strategies |
| ArcanaPass : Tiered NFT Liquidity | Membership-gated NFT passes |
| Prometheus : AI Liquidity Orchestrator | Cross-division AI orchestration |
| Foundry : Pipeline : Liquidity Analytics | Analytics and forecasting pipeline |
| Linktree : Liquidity Gateway | Unified multi-chain gateway |
| Positive Change : Corporate Modules | Risk management and compliance |

---

## Documentation

- [Architecture overview](ARCHITECTURE.md)
- [Setup and deployment guide](SETUP.md)
- [API reference](API.md)
- [Contributing guide](CONTRIBUTING.md)

---

## Contributing

See [`CONTRIBUTING.md`](CONTRIBUTING.md) for guidelines.

---

## License

[MIT](../LICENSE) © 2026 Positive Change Institute LLC
